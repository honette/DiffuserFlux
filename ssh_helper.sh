...
echo "rsync -avz --remove-source-files -e \"ssh -i ~/.ssh/id_runpod -p ${PORT}\" root@${IP}:/workspace/runpod-slim/ComfyUI/output/video/ /mnt/g/AI/source_images/_vid/output/"
echo "sh comfyui/auto_rsync.sh ${IP} ${PORT}"
echo "----------------------------------"
