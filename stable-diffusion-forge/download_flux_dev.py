import os
import requests
from tqdm import tqdm

# 保存先ベースパス
BASE_DIR = "/workspace/stable-diffusion-webui-forge"

# Hugging Face トークンを環境変数から取得
HF_TOKEN = os.getenv("HF_TOKEN")
if not HF_TOKEN:
    raise ValueError("環境変数 HF_TOKEN が設定されていません！")

# 認証ヘッダ
HEADERS = {"Authorization": f"Bearer {HF_TOKEN}"}

# ダウンロード対象ファイル
files = {
    "models/Stable-diffusion/flux1-dev.safetensors":
        "https://huggingface.co/black-forest-labs/FLUX.1-dev/resolve/main/flux1-dev.safetensors",

    "models/Stable-diffusion/ae.safetensors":
        "https://huggingface.co/black-forest-labs/FLUX.1-dev/resolve/main/ae.safetensors",
    "models/Stable-diffusion/vae/diffusion_pytorch_model.safetensors":
        "https://huggingface.co/black-forest-labs/FLUX.1-dev/resolve/main/vae/diffusion_pytorch_model.safetensors",
    "models/Stable-diffusion/text_encoder/model.safetensors":
        "https://huggingface.co/black-forest-labs/FLUX.1-dev/resolve/main/text_encoder/model.safetensors",
    "models/Stable-diffusion/text_encoder_2/model-00001-of-00002.safetensors":
        "https://huggingface.co/black-forest-labs/FLUX.1-dev/resolve/main/text_encoder_2/model-00001-of-00002.safetensors",
    "models/Stable-diffusion/text_encoder_2/model-00002-of-00002.safetensors":
        "https://huggingface.co/black-forest-labs/FLUX.1-dev/resolve/main/text_encoder_2/model-00002-of-00002.safetensors",
    "models/Stable-diffusion/tokenizer/vocab.json":
        "https://huggingface.co/black-forest-labs/FLUX.1-dev/resolve/main/tokenizer/vocab.json",

    "models/Lora/aidmaNSFWunlock-FLUX-V0.2.safetensors":
        "https://drive.google.com/uc?id=1jdc7Qz7BMRqtTkdt49RWwqvHCc3qKo0c",
    "models/Lora/abs_nipples.safetensors":
        "https://drive.google.com/uc?id=1TfkCebZYPf9TgJeMc5KpOwjWvYCrAdzp",
    "models/Lora/breasts_out_fashion.safetensors": # 2BoutOvalFlux1
        "https://drive.google.com/uc?id=11Q7hj6Qik9VRwJ9IPFuGmRrQ52Ta0Nyv",
}

# 確実にディレクトリ作成
for rel_path in files.keys():
    dir_path = os.path.join(BASE_DIR, os.path.dirname(rel_path))
    os.makedirs(dir_path, exist_ok=True)

def download_file(url, dest):
    """Hugging Face ファイルをダウンロードして保存"""
    response = requests.get(url, headers=HEADERS, stream=True)
    response.raise_for_status()
    total = int(response.headers.get("content-length", 0))
    with open(dest, "wb") as f, tqdm(
        desc=os.path.basename(dest),
        total=total,
        unit="B",
        unit_scale=True,
        unit_divisor=1024,
    ) as bar:
        for chunk in response.iter_content(chunk_size=8192):
            if chunk:  # keep-alive対策
                size = f.write(chunk)
                bar.update(size)

# ダウンロード実行
for rel_path, url in files.items():
    dest_path = os.path.join(BASE_DIR, rel_path)
    if not os.path.exists(dest_path):
        print(f"⬇️ Downloading: {os.path.basename(dest_path)}")
        download_file(url, dest_path)
    else:
        print(f"✅ Already exists: {os.path.basename(dest_path)}")

print("\n🎉 All model files are ready!")
